...
class Tree:
    ...
    def print_sorted_have_one_children(self, node):

        if node.left_son:
            self.print_sorted_have_two_children(node.left_son)
        if (not node.right_son and node.left_son) or (not node.left_son and node.right_son):
            print(node.value)
        if node.right_son:
...
tree.print_sorted_have_one_children(tree.root)
